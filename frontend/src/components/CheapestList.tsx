...
import { isMixedReturn, optionHashFor } from '../lib/route.js';
...
export function CheapestList({
  ...
  if (options.length === 0) {
    // filtered-to-nothing is still a board state (same --surface-2/frame/LED-tag
    // recipe as EmptyState/ErrorState/OptionNotFoundState), not a bare message —
    // the board never falls back to a generic panel just because a filter is
    // active. The copy/role/button below are unchanged (test-asserted).
    return (
      <div className="state">
  ...
  return (
    // board-house: the fare board is the page's real content, so it gets the
    // heavy 2px frame (shared with the board-state screens) — the mobile
    // stacked-card transform below is purely a .board descendant style, so it
    // still renders inside this same housing at narrow widths.
            ...
            return (
              ...
              >
                <td data-label="Route">
                  ...
                  {/* mixed rows only: the return leg's own airports, honest about a
                      trip that isn't a symmetric round trip of the row above it */}
                  {isMixedReturn(o) && (
                    <div className="route-ret">
                      <span className="ret-label">ret</span> {o.return_origin} → {o.return_destination}
                    </div>
                  )}
                </td>
